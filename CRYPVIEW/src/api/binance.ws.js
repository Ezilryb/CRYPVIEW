...
//  ✓ Backoff exponentiel révisé : 1s → 2s → 4s → 8s → 30s max
//  ✓ CustomEvents window `ws-status-changed` (connected | reconnecting | disconnected)
//  ✓ Nettoyage complet des timers et sockets avant toute nouvelle instance
//
//  Règles cursorrules conservées :
...
// Backoff révisé — délais plus courts pour un feedback rapide.
// Ces valeurs surchargent WS_CONFIG.BASE_DELAY_MS localement.
const BACKOFF_BASE_MS = 1_000;   // 1s première tentative
const BACKOFF_MAX_MS  = 30_000;  // plafond
 ...
 *   - Détection de silence par pingTimeout (35s)
 *   - Reconnexion automatique en backoff exponentiel (1s → 30s)
 ...
 *
 * Écouter l'état depuis n'importe quel module :
...
export class WSManager {
  ...
  #url               = '';
  #reconnectAttempts = 0;
  #reconnectTimer    = null;  // setTimeout de reconnexion (backoff)
  ...
  #resetPingTimeout() {
    ...
    this.#pingTimeoutTimer = setTimeout(() => {
      ...
      showToast(
        `Silence WS détecté (>${PING_TIMEOUT_MS / 1000}s) — reconnexion en cours…`,
        'warning',
        4_000
      );
   ...
   * Délais : 1s → 2s → 4s → 8s → 16s → 30s (plafond).
   * S'arrête après MAX_RECONNECT_ATTEMPTS tentatives.
   * @private
   */
  #scheduleReconnect() {
    ...
    // Backoff exponentiel plafonné : 1s × 2^n, max 30s
    ...
    showToast(
      `WS déconnecté — reconnexion dans ${(delay / 1000).toFixed(0)}s… (${this.#reconnectAttempts}/${WS_CONFIG.MAX_RECONNECT_ATTEMPTS})`,
      'warning',
      delay
    );
