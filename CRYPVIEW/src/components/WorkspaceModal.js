...
//
//  Usage :
//    const modal = new WorkspaceModal(workspaceManager, {
//      getCurrentState: () => ({
//        layout: 'multi4',
//        panels: [ { sym, tf, indicators } ],
//        syncCrosshair: true, syncZoom: true,
//      }),
//      onApply: (workspace) => applyWorkspace(workspace),
//    });
//    modal.open();
// ============================================================
...
export class WorkspaceModal {
  ...
  #render() {
    ...
    if (saveSection) {
      saveSection.innerHTML = this.#manager.isFull
        ...
        : this.#tplSaveSection();
      if (!this.#manager.isFull) this.#bindSaveEvents();
    }
  }
  ...
  #buildCard(ws) {
    ...
    name.style.cssText = 'font-family:\'Syne\',sans-serif;font-weight:800;font-size:13px;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
    ...
    layoutBadge.style.cssText = 'background:rgba(0,200,255,.1);border:1px solid rgba(0,200,255,.2);color:#00c8ff;padding:1px 7px;border-radius:3px;font-size:8px;font-weight:700;letter-spacing:.4px;';
    layoutBadge.textContent   = LAYOUT_LABELS[ws.layout] ?? ws.layout;

    const date = document.createElement('span');
    const d = new Date(ws.usedAt ?? ws.createdAt);
    date.textContent = d.toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' });

    meta.append(layoutBadge, date);
    ...
    if (ws.syncCrosshair) {
      syncRow.appendChild(this.#miniChip('🎯 Crosshair'));
    }
    if (ws.syncZoom) {
      syncRow.appendChild(this.#miniChip('🔍 Zoom'));
    }
    ...
    applyBtn.style.cssText = `
      ...
      border-radius:4px;cursor:pointer;font-weight:700;transition:background .15s;
      letter-spacing:.04em;
    `;
    ...
    delBtn.addEventListener('mouseenter', () => { delBtn.style.background = 'rgba(255,61,90,.18)'; delBtn.style.borderColor = 'var(--red)'; });
    delBtn.addEventListener('mouseleave', () => { delBtn.style.background = 'rgba(255,61,90,.08)'; delBtn.style.borderColor = 'rgba(255,61,90,.25)'; });
  ...
  #miniChip(text) {
    ...
    el.style.cssText = 'font-size:7px;padding:1px 6px;border-radius:3px;background:rgba(0,255,136,.08);border:1px solid rgba(0,255,136,.15);color:var(--accent);letter-spacing:.3px;';
    el.textContent = text;
    return el;
  }
  ...
  #tplSaveSection() {
    return `
      <div style="display:flex;gap:8px;align-items:center;">
        <input id="ws-name-input" type="text" placeholder="Nom du workspace…" maxlength="40"
               autocomplete="off"
               style="flex:1;background:var(--bg);border:1px solid var(--border);color:var(--text);
                      padding:9px 12px;font-family:'Space Mono',monospace;font-size:11px;
        ...
        <button id="ws-save-btn"
                style="background:rgba(0,255,136,.1);border:1px solid rgba(0,255,136,.3);
                       color:var(--accent);padding:9px 16px;font-family:'Space Mono',monospace;
                       font-size:10px;border-radius:4px;cursor:pointer;transition:all .15s;
  ...
  #bindSaveEvents() {
    ...
    const focusStyle   = () => { input.style.borderColor = 'var(--accent)'; };
    const blurStyle    = () => { input.style.borderColor = 'var(--border)'; };
    ...
    const doSave = () => {
      ...
      if (!state) { showToast('Impossible de lire l\'état courant.', 'error'); return; }
