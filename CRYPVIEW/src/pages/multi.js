...
export class MultiChartView {
  ...
  /**
   * Connecte les callbacks onItemsChange de chaque panneau pour
   * propager automatiquement les modifications de tracés aux panneaux
   * affichant le même symbole.
   */
  #bindMTFDrawingSync() {
    this.#instances.forEach(inst => {
      if (!inst.drawing) return;
      const prev = inst.drawing.onItemsChange;
  ...
  /**
   * Lie l'Object Tree au panneau actif et relie quand l'actif change.
   */
  #bindObjectTree() {
    ...
    const orig = this.#setActive.bind(this);
    this.#setActive = (idx) => {
      orig(idx);
      if (tree.visible) rebind();
    };
  }
  ...
  /**
   * Capture l'état complet de la vue multi pour la sauvegarde.
   * @returns {{ layout, panels, syncCrosshair, syncZoom }}
   */
  ...
  /**
   * Applique un workspace chargé depuis le WorkspaceManager.
   * @param {import('../features/WorkspaceManager').Workspace} ws
   */
  ...
  /**
   * Ajoute un bouton Workspaces dans la barre de sync.
   */
  #addWorkspaceButton() {
    const toolbar = document.getElementById('sync-toolbar');
    if (!toolbar) return;
...
class MultiChartInstance {
  ...
  #initComponents(allSymbols) {
    ...
    this.drawing    = new ChartDrawing(
      ...
      $(`draw-canvas-${i}`),
      $(`draw-svg-${i}`),
      `${this.#drawKey}_${this.sym.toLowerCase()}`  // sym-based → MTF Sync automatique
    );
  ...
  #rebuildModules() {
    ...
    // V3.5 — Recrée le ChartDrawing avec clé sym-based pour MTF Sync
    this.drawing = new ChartDrawing(
      this.chart, this.cSeries,
      $(`draw-canvas-${this.idx}`),
      $(`draw-svg-${this.idx}`),
      `${this.#drawKey}_${this.sym.toLowerCase()}`  // sym-based → clé change si sym change
    );
  }
